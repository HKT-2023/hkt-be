...
export class SendgridLib {
  ...
  public async sendMailCreateAccount(
    userNameIdentity: string,
    userEmail: string,
    password: string
  ): Promise<boolean> {
    let forgotPasswordTemplate = fs.readFileSync(
      "src/libs/sendgrid-template/create-user.txt",
      "utf8"
    );
    ...
    try {
      await this.mailService.send({
        from: process.env.SENDGRID_FROM,
        to: userEmail,
        subject: `${userNameIdentity}, Here is your account information. Please sign in and explore Klaytn application`,
